...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // If we are replying in a thread in 1:1 conversation
...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        const results = await ctx.db
            .query('messages')
            .withIndex('by_channel_id_parent_message_id_conversation_id', q => q.eq('channelId', args.channelId).eq('parentMessageId', args.parentMessageId).eq('conversationId', _conversationId))
        ...
        let response = {
            ...
            page: await Promise.all(
                results.page.map(async (message) => {
                    ...
                    const reactionsWithoutMemberIdProperty = deduceReactions.map(({ memberId, ...rest }) => rest)
        ...
        response = {
            ...response,
            page: response?.page.filter((msg): msg is NonNullable<typeof msg> => msg != null)
        }

        console.log('bodythsi')
        return response
    }
})
