    ...
    return (
        <div className='flex items-center gap-1 mt-1 mb-1' >
                ...
                data.map(reaction => (
                    <Hint key={reaction._id} label={`${reaction.count} ${reaction.count == 1 ? 'person' : 'people'} reacted with ${reaction.value}`} >
                        <button onClick={() => onChange(reaction.value)} className={cn(
                            'h-6 px-2 rounded-full bg-slate-700/70 border border-transparent text-slate-800 flex items-center gap-x-1',
            ...
            <EmojiPopover hint='Add reaction' onEmojiSelect={emoji => onChange(emoji.native)} >
                <button title='Add reaction' className='h-7 px-3 rounded-full bg-slate-200/70 bordre border-transparent hover:border-slate-500 flex items-center gap-x-1 ' >
